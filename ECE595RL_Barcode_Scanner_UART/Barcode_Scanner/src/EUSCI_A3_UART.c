...
void EUSCI_A3_UART_Validate_Data(uint8_t TX_Buffer[], uint8_t RX_Buffer[])
{
    ...
    for (int i = 0; i < BUFFER_LENGTH; i++)
    {
        ...
        // a newline (i.e. \n) for each iteration
        printf("TX_Buffer[%d]: 0x%02X, RX_Buffer[%d]: 0x%02X\n", i, TX_Buffer[i], i, RX_Buffer[i]);
        ...
        {
            printf("There is a mismatch at index %d: TX_Buffer[%d] != RX_Buffer[%d]\n", i, i, i);
        }
    }
}
